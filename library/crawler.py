...
from library.command import Singleton_cmd
...
class Crawler:
    def __init__(self, city, timestamp, area):
        ...
        self.json_info = Singleton_cmd.json_parser(os.path.join(self.parent_dir, "config", "city_info.json"))[city]
        self.data_dir = os.path.join(self.parent_dir, "data", self.json_info["name"], f"{area}_{self.current_time}")
        ...
        self.database = os.path.join(self.data_dir, f"{self.current_time}.db")
        self.table_base_name = "data"
        self.host = self.json_info["host_url"]
        self.esf_url = self.json_info["ershoufang_url"]
        self.area_list = self.json_info["area"].keys()
        self.get_url_end_flag = False

    def env_init(self):
        if self.area != "全部区域" and self.area not in self.area_list:
            print(f"{self.area}不属于{self.city}区域, {list(self.area_list)}")
            sys.exit(1)
    ...
    def get_all_areas_from_city(self):
        ...
        ershoufang = soup.find('div', attrs={'data-role': 'ershoufang'})
        for a in ershoufang.find_all('div')[0].find_all('a'):
            if a.get_text in self.area_list:
